...
=== Lines Over Time
Show codebase size over time, including ABAP and CDS code.

image::img/lines_over_time.png[]

=== Inheritance over time

Show inheritance usage over time.
...
Gives an easy overview of which database tables are accessed throughout the codebase.

=== API Usage

Lists usage of various SAP provided APIs throughout the codebase. Along with their public, private and steampunk status, successors and suggested successors are also listed.

=== Object Classifications

Overview of Clean Core classifications, use this to identify and improve code that is not Clean Core compliant.

=== Tree Map, Lines of Code

Visual representation of lines of code per package in the repository. The larger the box, the more lines of code in that package. This can be used to identify large packages that may need refactoring.